...
ESX.RegisterServerCallback('koid-vehicleshop:buyCar', function (source, cb, data)
	if source ~= nil and source ~= -1 then
		...
		if result then
			...
			ESX.OneSync.SpawnVehicle(GetHashKey(data.model), data.pos, data.heading,{plate = data.plate}, function(vehicle)
				Wait(100)
...
ESX.RegisterServerCallback('koid-vehicleshop:setOwned', function(source, cb, props)
	if source ~= nil and source ~= -1 then
...
ESX.RegisterServerCallback('koid-vehicleshop:testDrive', function(source, cb, model)
...
ESX.RegisterServerCallback('koid-vehicleshop:removeBucket', function (source, cb, bucket)
	if source ~= nil and source ~= -1 then
